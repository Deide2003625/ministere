...
export default function PendingDemands() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await getRequest('/api/get_in_progress');
  ...
  const handleSelectAllClick = (checked) => {
    if (checked) {
      const newSelecteds = userList.map((n) => n.denomination);
  ...
  const handleDeleteMultiUser = (selected) => {
    const deleteUsers = userList.filter((user) => !selected.includes(user.denomination));
    setSelected([]);
    setUserList(deleteUsers);
  };

  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - (requestList ? requestList.length : 0)) : 0;

  const formatDate = (dateFromResponse) => dateFromResponse.split(' ')[0];

  return (
    <Page title="Demandes en attente">
